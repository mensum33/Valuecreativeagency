  ...
  return (
    <article className="card-premium group overflow-hidden flex flex-col">
      <Link
        ...
        className="relative block aspect-[5/4] overflow-hidden img-bw"
      >
        <ImageWithFallback
        ...
        <span className="absolute top-4 left-4 rounded-full bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wider text-black border border-border">
          {work.industry}
        </span>
      </Link>
      <div className="flex flex-1 flex-col p-6 md:p-7">
        <Link href={`/recent-work/${work.slug}`}>
          <h3 className="text-xl font-display font-semibold text-black group-hover:underline transition-colors">
            {work.title}
          </h3>
        </Link>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {work.services.map((s) => (
            <span key={s} className="rounded-full border border-border bg-lightgrey px-3 py-1 text-xs font-medium text-black">
              {s}
            </span>
          ))}
        </div>
