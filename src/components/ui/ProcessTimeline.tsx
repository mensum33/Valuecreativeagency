const steps = [
  ...
  { number: "04", title: "Approve your proof", description: "You approve the final artwork before production starts." },
    ...
    <div className="relative">
      ...
      <div className="grid gap-6 md:gap-8">
        {steps.map((step, i) => (
          <div
            key={step.number}
            className="relative flex gap-5 md:gap-8 animate-fade-in-left"
            style={{ animationDelay: `${i * 0.1}s` }}
          >
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-navy text-sm font-bold text-white z-10">
              {step.number}
            </div>
            <div className="card-premium flex-1 p-5 md:p-6">
              <h3 className="text-lg font-display font-semibold text-navy">{step.title}</h3>
